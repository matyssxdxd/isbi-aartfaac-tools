from astropy.coordinates.solar_system import HAS_JPLEPHEM
import numpy as np
...
chan_defs = [
    {'freq': 6656.94e6, 'sideband': 'L', 'pol': 'RCP'},
    {'freq': 6656.94e6, 'sideband': 'L', 'pol': 'LCP'},
    {'freq': 6656.94e6, 'sideband': 'U', 'pol': 'RCP'},
    {'freq': 6656.94e6, 'sideband': 'U', 'pol': 'LCP'},
    {'freq': 6672.94e6, 'sideband': 'L', 'pol': 'RCP'},
    {'freq': 6672.94e6, 'sideband': 'L', 'pol': 'LCP'},
    {'freq': 6672.94e6, 'sideband': 'U', 'pol': 'RCP'},
    {'freq': 6672.94e6, 'sideband': 'U', 'pol': 'LCP'},
    {'freq': 6688.94e6, 'sideband': 'L', 'pol': 'RCP'},
    {'freq': 6688.94e6, 'sideband': 'L', 'pol': 'LCP'},
    {'freq': 6688.94e6, 'sideband': 'U', 'pol': 'RCP'},
    {'freq': 6688.94e6, 'sideband': 'U', 'pol': 'LCP'},
    {'freq': 6704.95e6, 'sideband': 'L', 'pol': 'RCP'},
    {'freq': 6704.95e6, 'sideband': 'L', 'pol': 'LCP'},
    {'freq': 6704.95e6, 'sideband': 'U', 'pol': 'RCP'},
    {'freq': 6704.95e6, 'sideband': 'U', 'pol': 'LCP'},
]
...
def generate_vdif_file(sample_rate, samples_per_frame, n_chan, n_thread, complex_data, bps,
    ...
    for frame_idx in tqdm(range(n_frames), desc="Writing frames"):
        ...
        for ch_idx, ch_def in enumerate(chan_defs):
            pol = (ch_def['pol'] == 'LCP')
            if ch_def['sideband'] == 'L':
                freq = ch_def['freq'] - 8e6
            else:
                freq = ch_def['freq'] + 8e6
            frame_data[:, ch_idx] = quantize_2bit(generate_sine_wave(freq, (t + delay), phase, pol))
        fw.write(frame_data)
...
if __name__ == "__main__":
    ...
    STATION = 0
    DURATION = 20  # seconds
    OUTPUT_FILE_1 = "./sin_test_data_1.vdif"
    ...
    # generate_vdif_file(SAMPLE_RATE, SAMPLES_PER_FRAME, N_CHANNELS, N_THREADS, COMPLEX_DATA, BPS, EDV, STATION, START_TIME, DURATION, BASE_FREQ, OUTPUT_FILE_2, 0.2, 0.0);
