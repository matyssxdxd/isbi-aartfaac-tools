...
#
    ...
    def scan_delays(self, scan: str) -> Dict[str, np.ndarray]:
        ...
        # clock_epoch = self.__parse_vex_time("2024y289d13h39m17s")
        ...
        for i in range(len(delays['Ib'])):
            print(f"total_delay={delays['Ir'][i] - delays['Ib'][i]}")
            print(f"geo_delay={g_delays['Ir'][i] - g_delays['Ib'][i]}")
    ...
    def write_scan_config(self, scan: str) -> None:
        ...
        #delays = [delays[0][60:], delays[1][60:]]

        # diff_ir = np.array(delays['Ir']) - np.array(sfxc_delays[0])
        # diff_ib = np.array(delays['Ib']) - np.array(sfxc_delays[1])

        # fig, (ax1) = plt.subplots(1, 1, figsize=(10, 8), sharex=True)

        # ax1.plot(delays['Ib'], label='Ib')
        # ax1.plot(delays['Ir'], label='Ir')
        # ax1.plot(sfxc_delays[0], label='Ir SFXC')
        # ax1.plot(sfxc_delays[1], label='Ib SFXC')
        # ax1.legend()

        # plt.suptitle(f"Difference Ir {np.mean(diff_ir)}, Difference Ib {np.mean(diff_ib)}")
        # plt.show()

        # print('IR')
        # for i in range(183):
        #     sfxc_ir = sfxc_delays[0][i]
        #     calc11_ir = delays['Ir'][i]
        #     print(f'Calc11={calc11_ir}, SFXC={sfxc_ir}, diff={np.abs(calc11_ir) - np.abs(sfxc_ir)}')

        # print('IB')
        # for i in range(183):
        #     sfxc_ib = sfxc_delays[1][i]
        #     calc11_ib = delays['Ib'][i]
        #     print(f'Calc11={calc11_ib}, SFXC={sfxc_ib}, diff={np.abs(calc11_ib) - np.abs(sfxc_ib)}')
