...
class SFXCData:	    
  ...
  def _parse_global_header(self):
    ...
    # the header format
    if self.global_header_size >= 92:
      # buf is bytes; split on the null byte and decode to str
      raw_parts = buf[92:].split(b'\0')
      # decode and filter out empty entries
      parts = [p.decode('utf-8', errors='replace').strip() for p in raw_parts if p]
      self.exp_stations = parts[:h.n_stations]
      self.exp_sources = parts[h.n_stations:(h.n_stations + h.n_sources)]
